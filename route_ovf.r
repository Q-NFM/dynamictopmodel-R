# very simple discharge relationship that lineraly relate the dischrage out of each area
# to its current storgae (water depth)
                           ...
                           ichan=1)
{
  ...
  #    ex[ichan] <- ex[ichan]+ sum(ex.over*groups$area)/groups$area[ichan]
    ...
    # so we don't get tiny amounts of surface storage  causing this routine to be called repeatedly
    ex <- round(ex, 5)
  }
  return(ex)
}
