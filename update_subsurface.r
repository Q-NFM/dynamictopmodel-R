						              ...
						              log=NULL)
{
  ...
  for(inner in 1:ntt)
  {
    ...
    qdrain <- flows$qbf-qin 
  	...
  	# if the difference between upslope input and downslope drainage exceeds the maximum throughflow, the excess generates
  	# "return flow" to the surface
   	# rate of generation of return flow is excess over maximum of net filling of area
   	ret_fl <- -pmin(groups$qbf_max+qdrain, 0)
   	
   	# how much excess surface storage is generated in this time step. 
   	stores$ex <- stores$ex + ret_fl*dtt

   	# if(any(ret_fl>0)){
   	#   browser()
   	# }
   	# remove it from the net input (add to drainage) as has appeared on the surface!
   	qdrain <- qdrain + ret_fl
   	  
   	# any subsurface flow that exceeds the maximum is added to surface as
   	# qex <- qin - 
   	# if(any(qex >0))
   	# {
   	#   iex <- which(qex>0)
   	#   qin[iex] <- groups$qbf_max[iex]
   	#   # addutional surface 
   	#   
   	# }
